...
api.get_invoice_from_webhook(webhook)
...
Use `get_invoice_from_webhook()` for both intermediate
`invoice.status_changed` events and final `invoice.paid` events:

```python
invoice = api.get_invoice_from_webhook(webhook)

if invoice["status"] == "processing":
    # Update visible payment state, but do not fulfill the order.
    pass
elif invoice["status"] == "completed":
    # Fulfill the order exactly once.
    pass
```
...
`verify_invoice_paid()` accepts the current webhook envelope and legacy callback
objects. It retrieves the authoritative invoice from the API, requires
`completed` status, and compares the callback status and paid amount.
